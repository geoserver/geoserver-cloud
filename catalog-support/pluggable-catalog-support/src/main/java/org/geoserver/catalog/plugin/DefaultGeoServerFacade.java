...
public class DefaultGeoServerFacade implements GeoServerFacade {
    ...
    protected List<SettingsInfo> settings = new ArrayList<SettingsInfo>();
    ...
    public <T extends ServiceInfo> T getService(String id, Class<T> clazz) {
        for (ServiceInfo si : services) {
            if (id.equals(si.getId())) {
                return ModificationProxy.create(clazz.cast(si), clazz);
            }
        }
    ...
    <T extends ServiceInfo> T find(
        ...
        for (ServiceInfo si : services) {
            if (clazz.isInstance(si) && Objects.equals(workspace, si.getWorkspace())) {
                return ModificationProxy.create(clazz.cast(si), clazz);
            }
        }
    ...
    <T extends ServiceInfo> T findByName(
        ...
        for (ServiceInfo si : services) {
            if (name.equals(si.getName()) && Objects.equals(workspace, si.getWorkspace())) {
                return ModificationProxy.create(clazz.cast(si), clazz);
            }
        }
    ...
    public List<ServiceInfo> filter(List<ServiceInfo> services, WorkspaceInfo workspace) {
        ...
        for (ServiceInfo si : services) {
            if (Objects.equals(workspace, si.getWorkspace())) {
                list.add(si);
            }
        }
        return list;
