...
The `gscloud/3.0.x/integration` branch of the [camptocamp/geoserver](https://github.com/camptocamp/geoserver) repository is checked out as a shallow submodule under `geoserver_submodule/geoserver`, and each *GeoServer Cloud* branch pins the submodule branch matching its GeoServer version.
...
`make deps-sync` fetches the head of `gscloud/3.0.x/integration` and checks it out, leaving the submodule at a commit the superproject does not record yet. Build it with `make deps`, then stage and commit the submodule to pin the new GeoServer revision.
