...
/**
 * {@link GlobalFilter} working in tandem with {@link GatewaySharedAuthenticationPreFilter} to enable sharing the webui
 ...
 *
 * <p>This is the post-filter in the above mentioned workflow, and takes care of updating the {@link WebSession} to
 * either store or remove the {@literal x-gsc-username} and {@literal x-gsc-roles} webui response headers, or clear them
 * out from the session when the webui responds with an empty string for {@literal x-gsc-username}.
 *
 * @since 1.9
...
public class GatewaySharedAuthenticationPostFilter implements GlobalFilter, Ordered {
    ...
    /**
     * @return {@link Ordered#LOWEST_PRECEDENCE}, being a post-filter, means it'll run the first post-processing once
     *     the response is obtained from the downstream service
     */
    @Override
    public int getOrder() {
    ...
    /**
     * After the filter chain's run, if the proxied service replied with the user and roles headers, save them in the
     * session.
     *
     * <p>
     *
