...
class GwcRequestPathInfoFilterTest {
    ...
    void doFilter_delegatesToAdaptRequestAndChain() throws Exception {
        ...
        new GwcRequestPathInfoFilter().doFilter(request, response, chain);

        ArgumentCaptor<HttpServletRequest> captor = ArgumentCaptor.forClass(HttpServletRequest.class);
        verify(chain).doFilter(captor.capture(), same(response));
        HttpServletRequest adapted = captor.getValue();
        assertThat(adapted).isNotSameAs(request);
        assertThat(adapted.getServletPath()).isEqualTo("/gwc");
        assertThat(adapted.getPathInfo()).isEqualTo("/demo/layer:name");
    }
