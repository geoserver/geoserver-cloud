...
/**
 * A utility class providing {@link UnaryOperator} functions to wrap or unwrap objects with a {@link ModificationProxy}
 * for use in resolving facades.
 *
 * <p>This class offers factory methods to create operators that either decorate objects with a
 * {@link ModificationProxy} ({@link #wrap()}) or remove such proxies ({@link #unwrap()}). These operators are designed
 * for use with {@link ResolvingCatalogFacade#setOutboundResolver(UnaryOperator)} or
 * {@link ResolvingCatalogFacade#setInboundResolver(UnaryOperator)}, enabling modification tracking or proxy removal in
 * catalog operations. The wrapping preserves object state changes, while unwrapping retrieves the underlying instance.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li><strong>Proxy Wrapping:</strong> {@link #wrap()} adds a {@link ModificationProxy} to objects if not already
 *       proxied, ensuring type safety via {@link ClassMappings}.
 *   <li><strong>Proxy Unwrapping:</strong> {@link #unwrap()} removes {@link ModificationProxy} wrappers, returning the
 *       raw object or null if the input is null.
 *   <li><strong>Null Safety:</strong> Both operators handle null inputs gracefully, returning null as appropriate.
 * </ul>
 *
 ...
 * }</pre>
 *
 * This configures the facade to wrap outbound {@link CatalogInfo} objects in a {@link ModificationProxy} and unwrap
 * inbound objects.
 *
 * @since 1.0
...
public class ModificationProxyDecorator {
     ...
     *
     * <p>The operator applies {@link #wrap(Object)} to each input, adding a proxy if the object isn’t already proxied,
     * or returning it unchanged if it is or if null.
     *
     ...
     *
     * <p>The operator applies {@link #unwrap(Object)} to each input, removing any {@link ModificationProxy} wrapper or
     ...
     *
     * <p>Checks if the input is non-null and not already a {@link ModificationProxy}. If so, it determines the
     * appropriate catalog interface via {@link ClassMappings} and creates a proxy using
     * {@link ModificationProxy#create(Object, Class)}. Returns null if the input is null, or the original object if
     * already proxied.
     *
     * @param <T> The type of object to wrap (typically a {@link CatalogInfo} subtype).
     ...
     *
     * <p>If the input is null, returns null. Otherwise, uses {@link ModificationProxy#unwrap(Object)} to remove any
     * proxy wrapper and return the raw object.
     *
     * @param <T> The type of object to unwrap (typically a {@link CatalogInfo} subtype).
