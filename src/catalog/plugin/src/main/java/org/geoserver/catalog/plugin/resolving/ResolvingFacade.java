...
/**
 * Interface that defines methods for applying resolution functions to objects of type {@code T} both inbound and
 * outbound, enabling customizable object transformation in a catalog context.
 *
 * <p>This interface provides a generic framework for processing objects (typically {@link CatalogInfo}) entering and
 * exiting a facade, using configurable {@link UnaryOperator} functions. The outbound resolver transforms objects before
 * they are returned to the caller, while the inbound resolver processes objects received from the caller before they
 * are passed to the underlying implementation. By default, both resolvers are set to the
 * {@link UnaryOperator#identity() identity} function, meaning no transformation occurs unless explicitly configured via
 * {@link #setOutboundResolver(UnaryOperator)} or {@link #setInboundResolver(UnaryOperator)}.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li><strong>Flexible Resolution:</strong> Supports custom inbound and outbound transformations, such as resolving
 *       proxies or initializing properties.
 *   <li><strong>Null Safety:</strong> Requires resolvers to handle null inputs, allowing implementations to decide how
 *       to process or propagate nulls.
 *   <li><strong>Pipeline Composition:</strong> Encourages chaining functions to build complex resolution workflows
 ...
 * }</pre>
 *
 * This pipeline sets the catalog property (e.g., {@link ResourceInfo#setCatalog}), resolves {@link ResolvingProxy}
 * references, initializes null collections, and wraps objects in a {@link ModificationProxy} for outbound objects,
 * while unwrapping proxies for inbound objects.
 *
 * <p>Notes:
 *
 * <ul>
 *   <li>Resolvers must accept {@code null} as an argument and may return {@code null}, giving implementations
 *       flexibility in handling missing or unresolved objects.
 *   <li>Callers must ensure resolvers use the correct {@link Catalog} instance if required by the chained functions, as
 *       this interface remains agnostic to such dependencies.
 * </ul>
 *
...
public interface ResolvingFacade<T> {
     ...
     *
     * <p>This function transforms objects of type {@code T} right before they leave the facade, enabling custom
     * processing such as resolving references or adding properties. The default is {@link UnaryOperator#identity()}.
     *
     ...
     *
     * <p>This function transforms objects of type {@code T} received by the facade (e.g., for add or update operations)
     * before they are passed to the underlying implementation. The default is {@link UnaryOperator#identity()}.
     *
     ...
     *
     * <p>This function is applied to all objects of type {@code T} entering the facade before they are processed by its
     * methods.
     *
     ...
     *
     * <p>Processes the object using the configured outbound resolver, which may transform it (e.g., resolving
     * references) or return null if unresolved or filtered out. Implementations must call this method on every object
     ...
     * or return null if invalid. Implementations must call this method on every object received from the caller before
     * further processing.
     *
     * @param <C> The specific type of object (a subtype of {@code T}).
     * @param info The object to resolve; may be null.
