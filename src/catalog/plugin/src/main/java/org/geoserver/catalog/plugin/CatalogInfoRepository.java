 ...
 *
 * <p>This interface defines a null-safe, domain-driven design (DDD)-inspired "repository" API for storing and querying
 * plain {@link CatalogInfo} objects (e.g., workspaces, layers, styles) with precise semantics.
 *
 * <p>It serves as a low-level abstraction for catalog persistence, eschewing null arguments to ensure clear,
 * predictable behavior. Each method has unambiguous intent, avoiding magic or decision-making that is delegated to
 * higher-level abstractions like {@link CatalogFacade} or {@link org.geoserver.catalog.Catalog}.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li><strong>Null Safety:</strong> No method accepts null arguments; violations throw {@link NullPointerException}.
 *   <li><strong>Query Semantics:</strong> Specialized methods (e.g., {@link StyleRepository#findAllByNullWorkspace()})
 *       provide explicit alternatives rather than relying on null checks or overloading.
 *   <li><strong>Return Types:</strong> Single-result queries return {@link Optional}; multi-result queries return
 *       {@link Stream}, which must be closed to release resources.
 *   <li><strong>Resource Management:</strong> Streams implement {@link AutoCloseable}, requiring proper closure (e.g.,
 *       via try-with-resources) to avoid resource leaks in implementations using live connections.
 * </ul>
 *
 * <p>Specialized sub-interfaces (e.g., {@link WorkspaceRepository}, {@link LayerRepository}) extend this interface with
 * type-specific operations, tailoring the API to each catalog entity type.
 *
...
public interface CatalogInfoRepository<T extends CatalogInfo> {
     ...
     * {@link ResourceRepository} would return {@link ResourceInfo}, indicating it handles all subtypes like
     * {@link FeatureTypeInfo}, {@link CoverageInfo}, {@link WMSLayerInfo}, and {@link WMTSLayerInfo}.
     *
     ...
     *
     * <p>The object is deleted from the underlying storage, and associated resources may be cleaned up depending on the
     * implementation. The provided object remains unchanged in memory.
     *
     ...
     *
     * <p>This method applies the provided {@link Patch} to the repository’s copy of the object identified by
     * {@code value}, returning the updated object. The patch specifies only the properties to change, enabling
     * efficient updates (e.g., translating to an SQL {@code UPDATE} for specific columns) and supporting multi-version
     * concurrency control (MVCC) semantics. The original {@code value} object is not modified; the returned object may
     * be a new instance or the same instance, depending on the implementation.
     *
     ...
     *
     * <p>This method should be called when the repository is no longer needed to ensure proper cleanup (e.g., closing
     * database connections). Implementations may use this to free resources explicitly.
     */
    void dispose();
     ...
     *
     * <p>This default implementation uses {@link #findAll(Query)} with a query that includes all objects of the
     * repository’s content type ({@link #getContentType()}) and {@link Filter#INCLUDE}. The returned stream must be
     ...
     *
     * <p>This method looks up an object with the specified ID, optionally constrained by the given type. If
     * {@code clazz} is null, it returns the first match of any subtype of {@code T}.
     *
     ...
     *
     * <p>This method searches for an object by name within the repository’s scope, returning the first match of the
     * given type, or an empty {@link Optional} if none is found.
     *
     * @param <U> The specific type of {@link CatalogInfo} to retrieve.
     ...
     *
     * <p>This method indicates whether the underlying storage can sort results by the given property name, useful for
     * optimizing queries with {@link SortBy}.
     *
     * @param propertyName The name of the property to check; must not be null. Nested properties are specified with a
     *     {@code .} separator, for example: {@code workspace.id}, {@code resource.store.namespace.prefix}, etc.
     ...
     *
     * <p>This method copies all objects from this repository to the target, potentially overwriting existing entries in
     * the target. It is intended for backup or migration purposes. Note: Future enhancements could include a progress
     * listener or cancellation mechanism.
     *
     * <p>Additionally, a {@code synchFrom} method might be more appropriate, as concrete implementations would know
     * better which optimizations to apply.
     *
