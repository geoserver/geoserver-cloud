...
public class LiteralDeserializer extends JsonDeserializer<Literal> {
    ...
    private static List<Object> convertArrayNodeToArrayList(
            Class<?> contentType, DeserializationContext ctxt, ArrayNode valuesArrayNode) throws IOException {
        List<Object> value;
        ...
        for (JsonNode node : valuesArrayNode) {
            if (node instanceof NullNode) {
                ...
                // finally parse the collection with the now known content type
                Object item = ctxt.readTreeAsValue(node, contentType);
                valuesList.add(item);
            }
        }
        value = valuesList;
