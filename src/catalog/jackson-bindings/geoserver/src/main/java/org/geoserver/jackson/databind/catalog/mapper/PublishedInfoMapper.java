...
public interface PublishedInfoMapper {
    ...
    default PublishedInfo map(PublishedInfoDto dto) {
        if (dto == null) {
            return null;
        } else if (dto instanceof LayerInfoDto l) {
            return map(l);
        } else if (dto instanceof LayerGroupInfoDto lg) {
            return map(lg);
        }
    ...
    default PublishedInfoDto map(PublishedInfo info) {
        if (info == null) {
            return null;
    ...
    LayerInfo map(LayerInfoDto o);

    LayerInfoDto map(LayerInfo o);

    @Mapping(source = "layers", target = "layers", qualifiedByName = "publishedInfo")
    LayerGroupInfo map(LayerGroupInfoDto o);

    LayerGroupInfoDto map(LayerGroupInfo o);
