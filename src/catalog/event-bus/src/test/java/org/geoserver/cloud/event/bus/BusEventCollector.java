...
public class BusEventCollector {
    ...
    public <T extends InfoEvent> RemoteGeoServerEvent expectOne(Class<T> payloadType, Predicate<T> filter) {
        ...
        List<RemoteGeoServerEvent> matches = await().atMost(Duration.ofSeconds(500)) //
    ...
    public <T extends InfoEvent> List<RemoteGeoServerEvent> allOf(Class<T> payloadEventType, Predicate<T> eventFilter) {
    ...
    private <T extends InfoEvent> Stream<RemoteGeoServerEvent> capturedEvents(Class<T> payloadType) {
