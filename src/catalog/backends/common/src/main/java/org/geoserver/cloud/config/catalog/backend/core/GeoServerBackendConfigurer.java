 ...
 * <h2>Overview</h2>
 *
 * <p>While vanilla GeoServer defaults to file-based "data directory" storage, GeoServer Cloud requires explicit backend
 * configuration through Spring Boot's externalized configuration properties. This marker interface identifies
 * configuration classes that provide complete catalog and configuration storage implementations for specific backend
 * technologies (e.g., data directory, PostgreSQL).
 *
 * <h2>Usage</h2>
 *
 * <p>Backend implementations should extend this class and be annotated with {@code @Configuration(proxyBeanMethods =
 * false)}. This allows implementations to use modern Spring dependency injection patterns with constructor or method
 * parameter injection, avoiding the limitations of {@code proxyBeanMethods = true} where bean methods must call each
 * other directly.
 *
 * <h2>Required Beans</h2>
 *
 * <p>A backend configuration class extending {@link GeoServerBackendConfigurer} must provide the following core beans
 * to establish a complete GeoServer catalog and configuration storage backend:
 *
 * <ul>
 ...
 *
 * <p>Backend configuration classes should be registered as Spring Boot auto-configurations by adding them to
 * {@code META-INF/spring/org.springframework.boot.autoconfigure.AutoConfiguration.imports}. They should implement
 * conditional activation using annotations such as:
 *
 * <ul>
