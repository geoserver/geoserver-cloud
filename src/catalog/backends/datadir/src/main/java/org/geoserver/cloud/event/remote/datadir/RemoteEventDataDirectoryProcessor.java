...
/**
 * Spring event listener that synchronizes the local data directory catalog with remote catalog and
 * configuration events broadcast over the distributed event bus.
 *
 * <p>In a GeoServer Cloud cluster, catalog modifications on any node are broadcast as events to all
 * other nodes. This processor listens for those remote events and applies the corresponding
 * operations to the local catalog, ensuring all nodes maintain synchronized state.
 *
 * <p>Handles {@link InfoAdded} events to create catalog/config objects (workspaces, layers,
 * services, etc.), {@link InfoModified} events to update existing objects via patch application,
 * {@link InfoRemoved} events to remove catalog/config objects, {@link DefaultWorkspaceSet},
 * {@link DefaultNamespaceSet}, and {@link DefaultDataStoreSet} events to update default entity
 * references, and {@link UpdateSequenceEvent} to synchronize the GeoServer update sequence counter.
 *
 * <p>When processing add or update events, the objects may contain references to other objects not
 * yet present locally (e.g., a Layer referencing a Resource that hasn't arrived yet). For default
 * entity events, the processor intentionally creates {@link ResolvingProxy} instances, allowing the
 * catalog facade to defer the operation if eventual consistency enforcement is enabled. This ensures
 * operations succeed once all dependencies arrive.
 *
 * <p>Each event listener checks {@code event.isLocal()} and ignores local events to prevent
 * processing events generated by this node, which would be redundant since the catalog operation
 * already occurred locally.
 *
 * @since 1.0
...
class RemoteEventDataDirectoryProcessor {
     ...
     *
     * <p>The update sequence is a monotonically increasing counter that tracks configuration changes.
     * This method ensures the local counter reflects the highest value seen across the cluster by
     * only updating when the remote sequence is greater than the local value.
     *
     ...
     *
     * <p>Removes catalog objects (workspaces, layers, etc.) or configuration objects (services,
     * settings). If the object is not found locally, a warning is logged but processing continues,
     * as the object may have already been removed by a previous event or never existed locally.
     *
     ...
     *
     * <p>The event carries the full object payload, which is added to the appropriate facade (catalog
     * or configuration). If the object contains unresolved references and eventual consistency is
     * enabled, the catalog facade will defer the operation until dependencies arrive.
     *
     ...
     *
     * <p>The event carries a {@link Patch} containing the modified properties. For catalog objects,
     * the patch is applied via the facade's update method. For configuration objects, the patch is
     * applied directly to the unwrapped object. If the patch contains unresolved references and
     * eventual consistency is enabled, the operation will be deferred.
     *
     * <p>Default entity set events (workspace, namespace, data store) are handled by dedicated
     * listeners and filtered out here.
     *
     ...
     *
     * <p>Intentionally creates a {@link ResolvingProxy} for the workspace reference, allowing the
     * catalog facade to defer the operation if the workspace hasn't arrived yet and eventual
     ...
     *
     * <p>Intentionally creates a {@link ResolvingProxy} for the namespace reference, allowing the
     * catalog facade to defer the operation if the namespace hasn't arrived yet and eventual
     * consistency is enabled.
     *
     ...
     *
     * <p>Intentionally creates {@link ResolvingProxy} instances for both the workspace and store
     * references, allowing the catalog facade to defer the operation if either object hasn't arrived
     * yet and eventual consistency is enabled.
     *
