...
public abstract class GeoServerEvent implements Serializable {
    ...
    /**
     * {@code getObjectId()} object identifier for changes performed to the {@link Catalog} itself (e.g.
     * {@code defaultWorkspace} and the like)
     */
    protected static final String CATALOG_ID = "catalog";

    /**
     * {@code getObjectId()} object identifier for changes performed to the {@link GeoServerInfo global config} itself
     * (e.g. {@code updateSequence} and the like)
     */
    private static final String GEOSERVER_ID = "geoserver";

    /** {@code getObjectId()} object identifier for changes performed to the {@link LoggingInfo} config */
