 ...
 *
 * <p>This filter extracts various properties from the HTTP request and adds them to the MDC based on the configuration
 * defined in {@link HttpRequestMdcConfigProperties}. The information can include:
 *
 * <ul>
 *   <li>Request ID
 *   <li>Remote address
 ...
 * </ul>
 *
 * <p>The filter adds these properties to the MDC before the request is processed, making them available to all logging
 * statements executed during request processing.
 *
...
public class HttpRequestMdcFilter extends OncePerRequestFilter {
     ...
     *
     * <p>This method adds HTTP request-specific information to the MDC before allowing the request to proceed through
     * the filter chain. The properties are added in a try-finally block to ensure they're available throughout the
     * request processing, even if an exception occurs.
     *
     ...
     *
     * <p>This method extracts various properties from the HTTP request and adds them to the MDC based on the
     * configuration in {@link HttpRequestMdcConfigProperties}. It handles lazy evaluation of properties through
     * suppliers to avoid unnecessary computation.
     *
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide a MultiValueMap containing the request
     * parameters. Using a Supplier allows lazy evaluation of the parameters.
     *
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide a MultiValueMap containing the request
     * cookies. Using a Supplier allows lazy evaluation of the cookies.
     *
     * @param req the HTTP servlet request
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide the session ID if a session exists, or null
     * otherwise. Using a Supplier allows lazy evaluation and avoids creating a new session if one doesn't exist.
     *
     * @param req the HTTP servlet request
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide the HTTP headers. The result is memoized
     * (cached) to avoid repeated computation if the headers are accessed multiple times.
     *
     * @param req the HTTP servlet request
     ...
     *
     * <p>This method creates a Guava Supplier that, when invoked, will extract all headers from the request and place
     * them in a Spring HttpHeaders object.
     *
     * @param req the HTTP servlet request
     ...
     *
     * <p>This method retrieves all values for a given header name from the request and returns them as a List of
     * Strings.
     *
