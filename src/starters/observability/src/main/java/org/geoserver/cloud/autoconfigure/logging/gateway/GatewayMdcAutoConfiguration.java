 ...
 * Spring Cloud Gateway uses its own filter chain mechanism with important differences:
 *
 * <ul>
 *   <li><b>Separate Filter Chains:</b> Spring WebFlux uses {@code WebFilter} while Spring Cloud Gateway uses
 *       {@code GlobalFilter}. These are two distinct filter chains that execute independently.
 *   <li><b>Execution Order:</b> The GlobalFilter chain executes before the WebFilter chain in Gateway, so
 *       Gateway-specific components (routes, predicates, custom filters) would miss MDC context without proper
 *       integration.
 *   <li><b>Complete Request Tracing:</b> To ensure consistent logging across the entire request lifecycle, MDC context
 *       must be available from the earliest point of request processing.
 * </ul>
 *
 * <p>Without these adapters, you might see logs from the WebFlux filter chain with MDC context, but logs from Gateway
 * components would lack this context, resulting in incomplete or inconsistent logging.
 *
 * <p>This configuration is conditional on the presence of MDCWebFilter and Spring Cloud Gateway classes, which are
 * typically provided by the {@code gs-cloud-starter-observability} module's optional dependency on Spring Cloud
 * Gateway.
 *
 * <p>It creates adapter filters that bridge between Spring WebFlux's WebFilter interface and Spring Cloud Gateway's
 * GlobalFilter interface, ensuring complete MDC context propagation.
 */
...
public class GatewayMdcAutoConfiguration {
     ...
     *
     * <p>This adapter allows the Spring WebFlux MDCWebFilter to be used as a Spring Cloud Gateway GlobalFilter,
     * ensuring MDC context is properly propagated throughout the gateway filter chain.
     *
     ...
     *
     * <p>This configuration is used by the Gateway's access log filter and is separate from any config used by WebFlux
     * filters.
     *
     * <p>This bean is primary to ensure it takes precedence over any other AccessLogFilterConfig beans that might exist
     * in the context.
     *
     ...
     *
     * <p>This filter logs HTTP requests processed by Spring Cloud Gateway. It uses its own instance of
     * AccessLogWebfluxFilter internally.
     *
     * <p>This is the primary access log filter for Gateway applications. The standard WebFlux filter in
     * AccessLogWebFluxAutoConfiguration is automatically disabled in Gateway applications to avoid double-logging.
     *
     ...
     *
     * <p>This adapter allows AccessLogWebfluxFilter to be used in the GlobalFilter chain. This adapter is the only
     * access log filter in Gateway applications because the standard WebFlux filter is disabled
     * by @ConditionalOnMissingClass in AccessLogWebFluxAutoConfiguration to prevent double-logging.
     */
    @RequiredArgsConstructor
