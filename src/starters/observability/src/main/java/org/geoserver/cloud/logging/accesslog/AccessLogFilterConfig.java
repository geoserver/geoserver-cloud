...
/**
 * Configuration to set white/black list over the request URL to determine if the access log filter will log an entry
 * for it.
 */
@Data
...
public class AccessLogFilterConfig {
     ...
     *
     * <p>Requests with URLs matching any of these patterns will be logged at TRACE level if trace logging is enabled.
     ...
     *
     * <p>Requests with URLs matching any of these patterns will be logged at DEBUG level if debug logging is enabled.
     * These patterns should follow Java's regular expression syntax.
     *
     * <p>Example configuration in YAML:
     *
     ...
     *
     * <p>Requests with URLs matching any of these patterns will be logged at INFO level. These patterns should follow
     ...
     *
     * <p>This method determines the appropriate log level for the given URI by checking it against the configured
     * patterns. It then logs the request details (method, status code, URI) at that level. If no patterns match, the
     * request is not logged.
     *
     * <p>The log format is: {@code METHOD STATUS_CODE URI}
     *
     * <p>Example log output: {@code GET 200 /api/data}
     *
     ...
     *
     * <p>This method checks the URI against the configured patterns for each log level (info, debug, trace) and returns
     * the highest applicable level. If no patterns match or if logging at the matched level is disabled, it returns
     * Level.OFF.
     *
     * <p>The level is determined in the following order of precedence:
     *
     * <ol>
     *   <li>INFO - if info patterns match and info logging is enabled
     *   <li>DEBUG - if debug patterns match and debug logging is enabled
     *   <li>TRACE - if trace patterns match and trace logging is enabled
     *   <li>OFF - if no patterns match or logging at the matched level is disabled
     * </ol>
     *
     ...
     *
     * <p>This method checks if the given URI matches any of the configured patterns for info, debug, or trace level
     * logging. If it matches any pattern, the request should be logged (although whether it is actually logged depends
     * on the enabled log levels).
     *
     * <p>This is a quick check used by the filter to determine if a request needs detailed processing for logging.
     *
     * @param uri the request URI to check
     ...
     *
     * <p>This method tests the provided URL against each pattern in the list. If any pattern matches, the method
     * returns true. If the pattern list is null or empty, it returns false.
     *
     * <p>The comparison is done using {@link java.util.regex.Pattern#matcher(CharSequence).matches()}, which checks if
     * the entire URL matches the pattern.
     *
