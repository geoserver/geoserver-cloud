 ...
 *
 * <p>This filter is responsible for populating the MDC with request-specific information in WebFlux reactive
 * applications. Since WebFlux can execute requests across different threads, the standard thread-local based MDC
 * approach doesn't work. Instead, this filter uses Reactor Context to propagate MDC values through the reactive chain.
 *
 * <p>The filter captures information based on the configuration properties:
 *
 * <ul>
 *   <li>{@link AuthenticationMdcConfigProperties} - Controls user-related MDC attributes
 *   <li>{@link HttpRequestMdcConfigProperties} - Controls HTTP request-related MDC attributes
 *   <li>{@link SpringEnvironmentMdcConfigProperties} - Controls application environment MDC attributes
 * </ul>
 *
 * <p>This filter is designed to run with {@link Ordered#HIGHEST_PRECEDENCE} to ensure MDC data is available to all
 * subsequent filters and handlers in the request chain.
 *
...
public class MDCWebFilter implements OrderedWebFilter {
     ...
     *
     * <p>This filter is set to {@link Ordered#HIGHEST_PRECEDENCE} to ensure it executes before any other filters,
     * making MDC data available to all subsequent components in the request processing chain.
     *
     ...
     *
     * <ol>
     *   <li>Saves the initial MDC state (to preserve it after request processing)
     *   <li>Clears the current MDC
     *   <li>Sets MDC attributes based on the current request
     *   <li>Propagates the MDC map through the Reactor Context
     *   <li>Restores the original MDC after request processing
     * </ol>
     *
     * <p>By using {@link Mono#contextWrite(reactor.util.context.ContextView) contextWrite}, this filter ensures that
     * MDC data is available throughout the entire reactive chain, even across thread boundaries.
     *
     ...
     *
     * <p>This method populates the MDC with information from:
     *
     * <ul>
     *   <li>Application environment (e.g., application name, instance ID)
     *   <li>HTTP request details (e.g., method, URI, remote address)
     *   <li>Authentication principal (e.g., user ID) if available
     * </ul>
     *
     * <p>The attributes included are controlled by the respective configuration properties. After setting all the
     * attributes, the method captures the complete MDC state and returns it as a map wrapped in a Mono.
     *
     ...
     *
     * <p>This method extracts information from the HTTP request and adds it to the MDC based on the
     * {@link HttpRequestMdcConfigProperties} configuration. Information that can be added includes:
     *
     * <ul>
     *   <li>Request ID
     *   <li>Remote address
     ...
     * </ul>
     *
     * <p>Note: Some attributes available in Servlet applications are not available in WebFlux, such as remoteHost and
     * sessionId (commented out in the implementation).
     *
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide the raw path of the request URI. Using a
     * Supplier allows lazy evaluation of the URI path.
     *
     ...
     *
     * <p>This method returns a Supplier that, when invoked, will provide the raw query string of the request URI. Using
     * a Supplier allows lazy evaluation of the query string.
     *
     * @param req the HTTP request
