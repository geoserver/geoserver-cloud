 ...
 *
 * <p>In reactive applications using WebFlux, the standard SLF4J MDC (Mapped Diagnostic Context) cannot be used directly
 * because the reactive pipeline may not execute in the same thread that initiated the request. This class provides a
 * bridge between the Reactor Context and MDC.
 *
 * <p>It allows extracting MDC information from the Reactor Context chain, making it accessible to logging statements in
 * both reactive and non-reactive code. This is especially useful for access logging and other cross-cutting logging
 * concerns.
 *
 * <p>The MDC data is stored in the Reactor Context under the key {@link #MDC_CONTEXT_KEY}.
 */
@UtilityClass
@Slf4j
public class ReactorContextHolder {
     ...
     *
     * <p>This constant defines the key under which the MDC map is stored in the Reactor Context. It should be used
     * consistently across all code that needs to access or modify the MDC data in a reactive context.
     */
     ...
     *
     * <p>This method tries to get MDC information from the thread-local MDC context, which is typically set by
     * MDCWebFilter in WebFlux applications. If no MDC context is found, it returns an empty map.
     *
     * <p>This approach avoids blocking operations in reactive code while still providing access to MDC data for logging
     * purposes.
     *
     ...
     *
     * <p>This method provides a convenient way to transfer MDC values from a Reactor Context into the current thread's
     * MDC context before logging operations. It's particularly useful in operators like doOnNext, doOnEach, or
     * doOnSubscribe.
     *
     ...
     *
     * <p>This method can be used in hooks like doOnEach or doOnNext to ensure that MDC values from the reactor context
     * are set in the current thread for logging purposes.
     *
     ...
     *
     * <p>This method allows you to explicitly retrieve the MDC map from within a reactive chain without using blocking
     * operations.
     *
